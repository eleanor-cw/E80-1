...
void XYStateEstimator::updateState(imu_state_t * imu_state_p, gps_state_t * gps_state_p) {
  if (gps_state_p->num_sat >= N_SATS_THRESHOLD){
    ...
    state.x = RADIUS_OF_EARTH_M*cos(origin_lat*PI/180)*(gps_state_p->lon - origin_lon)*PI/180;
    state.yaw = angleDiff(imu_state_p->heading*-1*PI/180+PI/2);

    float heading_rad = imu_state_p->heading * PI / 180;
